...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    let ticket = match Ticket::new(title.clone(), description.clone(), status.clone()) {
        Ok(ticket) => ticket,
        Err(e) => match e.as_str() {
            "Description cannot be empty" | "Description cannot be longer than 500 characters" => Ticket::new(title, "Description not provided".to_string(), status).unwrap(),
            _ => panic!("{}", e),
        }
    };
    ticket
