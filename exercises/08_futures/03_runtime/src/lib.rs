    ...
    T: Display + Send + Sync + 'static,
{
    ...
    let handle1 = tokio::spawn(reply_inner(first, response.clone()));
    let handle2 = tokio::spawn(reply_inner(second, response));
    tokio::join!(handle1, handle2);
}


async fn reply_inner(listener: TcpListener, response: String) -> Result<(), anyhow::Error> {
    loop {
        ...
        tokio::spawn(async move {
            ...
            writer.write_all(format!("{}", &resp).as_bytes()).await.unwrap();
        });
    }
}
